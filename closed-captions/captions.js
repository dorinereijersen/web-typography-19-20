/*eslint-env browser*/

captions[0] = ['3.5','8.3'];
    //yes, sir
captions[1] = ['8.3','9.3'];
    //buzzer
captions[2] = ['9.3','10.7'];
    //recite your baseline
captions[3] = ['10.8','12.5'];
	//And blood-black nothingness began to spin
captions[4] = ['12.5','15.6'];
    //beeep
captions[5] = ['16.2','17.0']
captions[6] = ['17.369','20'];
captions[7] = ['20.1','23.3'];
	//Fuck off, skin-job!
captions[8] = ['23.3','25.2'];
	
captions[9] = ['26.8','31.6'];
captions[10] = ['32.7','33'];
captions[11] = ['33.2','34'];
	//Have you ever been in an institution?
captions[12] = ['34','36'];
captions[13] = ['36','36.3'];
captions[14] = ['36.5','37.3'];
	//Do they keep you in a cell?
captions[15] = ['37.3','38.6'];
captions[16] = ['38.6','38.9'];
captions[17] = ['39.1','39.9'];
	//When you're not performing your duties, do they keep you in a little box?
captions[18] = ['39.9','42.8'];
captions[19] = ['42.8','43.1'];
captions[20] = ['43.3','44.1'];
	//Interlinked
captions[21] = ['44','44.5'];
captions[22] = ['45','45.7'];
	//What's it like to hold the hand of someone you love?
captions[23] = ['45.7','47.6'];
captions[24] = ['47.6','48.1'];
captions[25] = ['48.3','49'];
	//Did they teach you how to feel, finger to finger?
captions[26] = ['49.1','50.9'];
captions[27] = ['50.9','51.4'];
captions[28] = ['51.6','52.3'];
	//Do you long for having your heart interlinked?
captions[29] = ['52.3','54'];
captions[30] = ['54','54.5'];
captions[31] = ['54.7','55.4'];
	//Do you dream about being interlinked?
captions[32] = ['55.4','57.2'];
captions[33] = ['57.2','58'];
	//What's it like to hold your child in your arms?
captions[34] = ['58','59.8'];
captions[35] = ['59.8','60.3'];
captions[36] = ['60.3','61'];
	//Do you feel that there's a part of you that's missing?
captions[37] = ['61','62.7'];
captions[38] = ['62.7','63.2'];
captions[39] = ['63.2','63.9'];
	//Within cells interlinked.
captions[40] = ['63.9','65.2'];
captions[41] = ['65.2','66.6'];
	//Why don't you say that three times: "within cells interlinked".
captions[42] = ['66.6','69.1'];

captions[43] = ['69.1','70.6'];
captions[44] = ['70.6','72.2'];
captions[45] = ['72.2','74.2'];
	// We're done.
captions[46] = ['77.1','78.7'];
captions[47] = ['80','81.738'];
captions[48] = ['82.2','84.3'];
captions[49] = ['85.4','87'];

//end manual labour
captions[50] = ['87.68','88.18'];
captions[51] = ['88.38','88.88'];
// Have you ever been in an institution. Cells
captions[52] = ['88.88','91.24'];
captions[53] = ['91.34','91.84'];
captions[54] = ['91.98','95.52'];
// -Cells
captions[55] = ['95.52','96.02'];
captions[56] = ['96.12','100.2'];
captions[57] = ['100.3','102.4'];
// -Interlinked
captions[58] = ['102.64','103.3'];
captions[59] = ['103.3','104.38'];
captions[60] = ['104.54','105.42'];
// Dreadfully
captions[61] = ['105.42','105.92'];
captions[62] = ['106','106.5'];
captions[63] = ['106.64','109.28'];
// -Dreadfully
captions[64] = ['109.28','109.8'];
captions[65] = ['109.8','112.4'];
captions[66] = ['112.4','113'];
// Dreadfully distinct
captions[67] = ['113','114.06'];
captions[68] = ['114.12','114.98'];
captions[69] = ['115.02','115.56'];
// -Dark
captions[70] = ['115.56','116.26'];
captions[71] = ['116.26','117.4'];
captions[72] = ['117.4','118.44'];
// Withing one stem
captions[73] = ['118.44','120'];
captions[74] = ['120.46','121.28'];
captions[75] = ['121.4','122.66'];
// -And dreadfully distinct
captions[76] = ['122.66','123.86'];
captions[77] = ['123.98','124.92'];
captions[78] = ['124.92','125.52'];
// A tall white fountain played
captions[79] = ['125.52','127.28'];
captions[80] = ['128.18','130.14'];
captions[81] = ['132.58','135'];